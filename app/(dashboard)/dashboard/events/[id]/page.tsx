...
import {CalendarDays,MapPin,Search,ChevronDown,ChevronRight,Play,} from "lucide-react";
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8 bg-white">
      ...
      <div className="flex items-center text-sm">
        <Link href="/dashboard/events" className="text-orange-600 hover:underline">
          Events
        </Link>
        <span className="mx-2 text-orange-400">{">"}</span>
        <Link
          href="/dashboard/events?type=free"
      ...
      <div className="bg-gray-50 p-6 rounded-lg">
        ...
        <p className="text-gray-600 mb-6">
          ...
          {Math.floor(totalDurationMinutes / 60)} mins total length
        </p>
      ...
      <div className="space-y-8">
        {Object.keys(sessions).map((session) => {
          const isOpen = openSessions[session] ?? false;
          return (
            <div key={session}>
              <button
                onClick={() => toggleSession(session)}
                className="flex items-center gap-3 mb-4 w-full text-left focus:outline-none"
              >
                {isOpen ? (
                  ...
                  {sessions[session].map((video, index) =>
                    isRegistered(id) ? (
                      ...
                      >
                        ...
                        <span className="text-black-500 text-sm font-mono whitespace-nowrap">
                          Video - {video.duration}
                        </span>
                      </div>
                    )
